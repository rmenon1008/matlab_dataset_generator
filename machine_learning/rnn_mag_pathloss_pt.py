...
d.ray_path_losses = d.unwrap(d.ray_path_losses)
scaler_path_loss = get_scaler('minmax')
paths = d.generate_straight_paths(NUM_PATHS, 10)
dataset_mag = d.paths_to_dataset_mag_only(paths)
dataset_path_loss = d.paths_to_dataset_pathloss_only(paths)
dataset_positions = d.paths_to_dataset_positions(paths)
cprint(f"dataset_mag.shape {dataset_mag.shape}")
cprint(f"dataset_path_loss.shape {dataset_path_loss.shape}")
dataset_interleaved = d.paths_to_dataset_interleaved_w_path_loss(paths)
...
X_train, X_test, y_train, y_test = train_test_split(dataset_interleaved[:,:9,:], dataset_interleaved[:,9:10,:].squeeze(), train_size = 0.95, shuffle=False) # get 9 points for training, the last point as test (in a 1d array)
X_train, X_val, y_train, y_val = train_test_split(X_train, y_train, train_size = 0.8, shuffle=False) # gets the training data for X and y and splits 80% for training and 20% for validation
# skipping one value, getting only the mag on each row for testing
y_train = y_train[:,0::2] 
y_test = y_test[:,0::2] 
y_val = y_val[:,0::2]
...
if DEBUG:
    ...
    for i in range(3):
        ...
        axs[0].set_title("CSI Mag and Path Loss Reading 7")
        # axs[0].plot(new_input[6,1::2])
        axs[1].plot(new_input[7,0::2])
        axs[1].set_title("CSI Mag and Path Loss Reading 8")
